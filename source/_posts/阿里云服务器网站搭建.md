...
categories:
  - 网络
  - 服务器
description: 一个适用于阿里云Linux系统（以及其兼容的CentOS等红帽系统）的静态Web服务搭建教程，涉及到单网站、多网站以及https
...
本教程基于LAMP结构搭建Web服务，且在本文中仅演示其中'L'（Linux）和'A'（Apache）的部分，也就是仅仅只是静态网站的搭建教程。

{% endnote %}

{% note warning %}

我所选择的用于演示的服务器是阿里云的一个 ECS 云服务器，地址在香港，所以不用备案域名就可以直接解析到该服务器；我所选择的服务器的系统是阿里巴巴针对云服务器所定制的一个 Linux 系统，版本号是：Alibaba Cloud Linux 3.2104 LTS 64位。如果你选用其他的系统，可能在下面介绍的一些细节的操作上存在一些区别，请自行判断。
...
我所使用的这个系统默认是不开防火墙的，也就是从外部就可以访问服务器所有的端口，这样肯定是不安全的，阿里云用了另外一种方式来管理服务器的入流量的允许端口，也就是安全组。由于Web应用默认使用 80 端口，如果是https则默认使用 443 端口，所以如果要搭建Web应用，最好是开启这两个端口，如果开启其他端口，在访问时还需要在域名后指定端口号，访问比较麻烦。
...
要连接到阿里云的服务器有两种方法，一种是直接通过阿里云提供的网络工具连接，另一种是直接从系统使用 ssh 连接。
...
- Windows
  ...
  > 如果在Windows上有Git Bash之类的终端也可以像Linux一样操作
...
在这个目录下，主要包括`conf`目录、`conf.d`目录以及`conf.modules.d`目录。在`conf`目录下的`httpd.conf`文件是 Apache 的主要配置文件，当我们运行 httpd 命令时，它就会读取该配置文件。
...
在这个目录下是 Apache 默认保存网页文件的地方，它有一个叫做`html`的目录，你可以把自己的网站文件全部放在这里面，运行 Apache 就直接能访问你的网页了。但我们之后会通过 Git 来拉取仓库，所以不用这个文件夹。
...
这一行内容表示将包含所有`conf.modules.d`目录下的所有`.conf`配置文件。
...
> 由于我的博客是用 hexo 搭建的，所以需要使用 nodejs，如果你不需要就不用安装。
...
# *:80 表示所有对该服务器的 80 端口访问
<VirtualHost *:80>
	ServerName syunn.cn
	# 设置别名，我希望 www.syunn.cn 可以和使用 syunn.cn 达到同样的效果
...
我们直接点击**立即购买**即可。需要注意的是免费证书一年只能购买 20 个，每个证书只能用于一个具体的域名，不能用于泛域名如`*.syunn.cn`（对我来说足够了）。另外这些证书有效期为 1 年，但是 1 年之后又可以重新免费获取 20 个的额度（简直不要太爽）。
...
安装好之后有一个非常重要的文件：`/etc/httpd/conf.d/ssl.conf`，该文件为 ssl 功能的配置文件，我们主要修改这个文件
...
由于一个域名将有多个证书文件，所以我的建议是在`/etc/httpd/`目录下建立一个`certs`目录，然后再按照域名来建立相应的子目录，将对应的证书内容放在对应的子目录下，这样不容易产生混乱。
...
<VirtualHost *:443>
	...
	# 以_public.crt 结尾的那个文件
	SSLCertificateFile /etc/httpd/certs/syunn/syunn.cn_public.crt
	# 以 .key 结尾的那个文件
	SSLCertificateKeyFile /etc/httpd/certs/syunn/syunn.cn.key
	# 以 _chain.crt 结尾的那个文件
	SSLCertificateChainFile /etc/httpd/certs/syunn/syunn.cn_chain.crt
